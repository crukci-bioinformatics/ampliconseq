...
# Collates and sorts amplicon coverage metrics.
...
option_list <- list(
  ...
  make_option(c("--amplicons"), dest = "amplicons_file",
              help = "Tab-separated file containing amplicon details the order of which is used in sorting the coverage metrics (ID column required)"),

  make_option(c("--output"), dest = "output_file",
              help = "Output table")
)

option_parser <- OptionParser(usage = "usage: %prog [options] amplicon_coverage_files", option_list = option_list, add_help_option = TRUE)
arguments <- parse_args(option_parser, positional_arguments = TRUE)

input_files <- arguments$args
if (length(input_files) == 0) stop("Amplicon coverage file(s) must be specified")
...
coverage <- map_dfr(input_files, read_tsv, col_types = cols(.default = "c"))
...
write_tsv(coverage, output_file, na = "")
